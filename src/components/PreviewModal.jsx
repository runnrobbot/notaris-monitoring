import { getPdfViewUrl } from '../utils/storageUtils'
          ...
          <Section title="Akad & Pajak">
            {[['tanggal_pinjam','Tanggal Pinjam'],['tanggal_diterima','Tanggal Diterima'],['tanggal_bayar_pajak','Tanggal Bayar Pajak'],['tanggal_akta','Tanggal Akta']].map(([k,lbl])=>(
              <div key={k} style={rowStyle}>
                ...
                <span style={{fontSize:11,color:record[k]?'var(--text-primary)':'var(--text-muted)'}}>{fmtDate(record[k])||'—'}</span>
              </div>
            ))}
          </Section>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={dateKey} className="p-3 rounded-lg space-y-2" style={{background:'var(--bg-raised)',border:'1px solid var(--border)'}}>
                  ...
                  <p style={{fontSize:11,color:record[dateKey]?'var(--text-primary)':'var(--text-muted)'}}>{fmtDate(record[dateKey])||'Belum ada tanggal'}</p>
                  {record[pdfKey]?.url ? (
                    <a href={getPdfViewUrl(record[pdfKey])} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-xs transition" style={{color:'var(--gold)'}}>
                      <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20" style={{color:'#e05252'}}>
                        <path d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z"/>
                      </svg>
                      Lihat Dokumen PDF
                    </a>
                  ) : (
                    <p style={{fontSize:11,color:'var(--text-muted)'}}>Tidak ada PDF</p>
                  )}
                </div>
              ))}
            </div>
